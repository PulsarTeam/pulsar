...
func send(count int, ip string) {
	...
	if err != nil {
		fmt.Println("eth_accounts Error", err.Error())
		msg <- false
		return
	}
	if len(account) == 0 {
	...
	// get balance
	var result string
	err = client.Call(&result, "eth_getBalance", account[from], "latest")
	if err != nil {
		fmt.Println("eth_getBalance Error:", err)
		msg <- false
		return
	}
	...
	fmt.Println("eth_getBalance Ok:", bl, result)
...
func sendTransaction(client *rpc.Client, account []string, wait *sync.WaitGroup) {
	...
	var result string
	client.Call(&result, "eth_sendTransaction", mapData)
	if result != "" {
		Count++
