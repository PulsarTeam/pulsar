...
func TestHeaderVerification(t *testing.T) {
	...
	chain, _ := NewDAGManager(testdb, nil, params.TestChainConfig, ethash.NewFaker(), vm.Config{})
	defer chain.Stop()
	...
	for i := 0; i < len(blocks); i++ {
		...
		chain.InsertBlocks(blocks[i : i+1])
	}
}
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewDAGManager(testdb, nil, params.TestChainConfig, ethash.NewFakeFailer(uint64(len(headers)-1)), vm.Config{})
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	chain, _ := NewDAGManager(testdb, nil, params.TestChainConfig, ethash.NewFakeDelayer(time.Millisecond), vm.Config{})
	defer chain.Stop()
